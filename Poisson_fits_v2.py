...
for totOD in totalODs:
    ...
    #sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotGFP",marker='o',color='limegreen',alpha = 0.5,s=60)
...
for totOD in totalODs:
    ...
    sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotGFP",marker='o',color='limegreen',alpha = 0.5,s=60)
    plt.errorbar(np.log10(ODvals),NotRFPMeanPerOD,NotRFPSDPerOD, fmt="^", color="k",mfc='orchid',mec='black', ms=6)
    sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotRFP",marker='o',color='orchid',alpha = 0.5,s=60)
...
for i, totOD in enumerate(totalODs):
    ...
    # plot data
    plt.errorbar(np.log10(ODvals),NotGFPMeanPerOD,NotGFPSDPerOD, fmt="o", color="limegreen",mfc='limegreen',mec='black', ms=5)
    sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotGFP",marker='o',color='limegreen',alpha = 0.4,s=40)
    plt.errorbar(np.log10(ODvals),NotRFPMeanPerOD,NotRFPSDPerOD, fmt="^", color="orchid",mfc='orchid',mec='black', ms=6)
    sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotRFP",marker='^',color='orchid',alpha = 0.4,s=40)
...
for i, totOD in enumerate(totalODs):
    ...
    # plot data
    plt.errorbar(np.log10(ODvals),NotGFPMeanPerOD,NotGFPSDPerOD, fmt="o", color="k",mfc=thiscolor,mec='black', ms=6)
    sns.scatterplot(data=thisODdata, x="log10_ODoneStrain", y="NotGFP",marker='o',edgecolor = 'w',color=thiscolor,alpha = 0.4,s=30)
    #plt.errorbar(np.log10(ODvals),NotRFPMeanPerOD,NotRFPSDPerOD, fmt="^", color="k",mfc='orchid',mec='black', ms=6)
    plt.ylabel('fraction of untransformed \n transformable cells')
    plt.title('Total OD = ' + str(totOD),color='k')
...
for idx, ODtot in enumerate(ODtots):
    ...
    fitYG = fractionNotTransformed(FitX, fit_alphaG)
    fitY_contG = fractionNotTransformed(xForFit_cont, fit_alphaG)
    RsqrdG = getRsquared(fitYG, FitYG)
    ...
    fitYR = fractionNotTransformed(FitX, fit_alphaR)
    fitY_contR = fractionNotTransformed(xForFit_cont, fit_alphaR)
    RsqrdR = getRsquared(fitYR, FitYR)
    ...
    plt.plot(np.log10(xForFit_cont),fitY_contR,'-',lw=1.5,color=thisColor, label='_nolegend_')
    #plt.errorbar(np.log10(ODvals),NotGFPMeanPerOD,NotGFPSDPerOD, fmt="o", color="k",mfc = thisColor, mec='black', ms=5)
    plt.errorbar(np.log10(ODvals),NotRFPMeanPerOD,NotRFPSDPerOD, fmt="o", color="k",mfc = thisColor, mec='black', marker=thisMarker, ms=5)
...
for idx, ODtot in enumerate(ODtots):
    ...
    # now do the plotting itself
    plt.errorbar(np.log10(ODvals),NotGFPMeanPerOD,NotGFPSDPerOD, fmt="o", color=thisColor,mfc = thisColor, mec='black',marker=thisMarker, ms=6)


#perform the Poisson fit for GFP
scaledAlpha = np.exp(GreenYintercept)
xForFit_cont = np.logspace(-4,-0.2,100) # dense OD values for plotting purposes
fitY_contG = fractionNotTransformed(xForFit_cont, scaledAlpha)
...
xForFit_cont = np.logspace(-4,-0.2,100) # dense OD values for plotting purposes
fitY_contR = fractionNotTransformed(xForFit_cont, scaledAlpha)
