...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  8 15:58:34 2024
...
def fractionTransformed(dilutionOD,  totalOD, ODsat, N): 
    # dilutionOD = the OD of the culture that we're diluting, the one with the nuclear tag
    # totalOD = the total bacterial OD
    # ODsat = a sclaing factor to go from OD to poisson probability   
    # N = number of transformation slots per plant cell
    if totalOD >= ODsat:
        p = dilutionOD/totalOD
    else:
        p = dilutionOD/ODsat        
    TransfP = 1 - (1-p)**N # Poisson probability of zero successes   
    return TransfP
...
#%% plot the data for each of the total ODs, incorporate 'fraction transformable' and Binomial fit
...
for i, totOD in enumerate(totalODs):
    ...
    # first, fit GFP
    FitBounds = ((totOD-0.0001,0.3,25),(totOD,0.5,35)) # lower and upper bounds for totalOD, ODsat and N
    poptG, pcov = scipy.optimize.curve_fit(fractionTransformed, FitX, FitYG, bounds = FitBounds)
    
    fit_ODsat = poptG[1]
    fit_N = poptG[2]
    print(fit_ODsat,fit_N)
    fit_ODsat_error = np.round(np.sqrt(np.diag(pcov))[0],1)
    fit_N_error = np.round(np.sqrt(np.diag(pcov))[1],2)
    print(fit_ODsat_error,fit_N_error)
    fitYG = fractionTransformed(FitX, totOD,fit_ODsat,fit_N)
    fitY_contG = fractionTransformed(xForFit_cont, totOD,fit_ODsat,fit_N)    
    # # now, fit  RFP
...
# #%% plot the data for each of the total ODs, fit 'fraction transformable' and Binomial fit 
...
#     fractionTransformed(FitX,  totOD, ODsat, N)
...
for idx, ODtot in enumerate(ODtots):
    ...
    #FitBounds = ((ODtot-0.0001,0.3,25),(ODtot,0.5,35)) # lower and upper bounds for totalOD, ODsat and N
    #poptG, pcov = scipy.optimize.curve_fit(fractionTransformed, FitX, FitYG, bounds = FitBounds)
    fit_ODsat = poptG[1]
    fit_N = poptG[2]
    alphaG_error = np.round(np.sqrt(np.diag(poptG))[0][0],1)
    fitYG = fractionTransformed(FitX, ODtot,fit_ODsat,fit_N)
    fitY_contG = fractionTransformed(xForFit_cont,ODtot,0.32,29)
    ...
    plt.errorbar(np.log10(ODvals),GFPMeanPerOD,GFPSDPerOD, fmt="o", color=thisColor,mfc = thisColor, mec='black',marker=thisMarker,ms=5,mew=0.5)
...
#concatenate the x values (the dilution ODs), the y values (the fraction transformed) and the total OD, of each datapoint
allXvalues = np.zeros(1)
allYvalues = np.zeros(1)
allTotalODs = np.zeros(1)
for totOD in totalODs:
    thisODdata = ODdata[ODdata['ODtot']==totOD]   
    #remove the ODtot = 0.2 that I had included in the ODtot=0.1 experiment
    if totOD == 0.1:
    ...
    if totOD == 0.5:
        thisODdata = thisODdata[thisODdata.OD != 1] 
    X = thisODdata['ODoneStrain'] # ****  IMPORTANT!  ***** the OD in the dataframe is the mix of green and red strains
    YG = thisODdata['fracGFP']
    totalOD = np.ones(len(X))*totOD
    allXvalues = np.concatenate((allXvalues,X))
    allYvalues = np.concatenate((allYvalues,YG))
    allTotalODs = np.concatenate((allTotalODs,totalOD))
allXvalues = allXvalues[1:]
allYvalues = allYvalues[1:]
allTotalODs = allTotalODs[1:]


def pTransWrapper(allXvalues, ODsat, N):
    Yfit = np.zeros(len(allXvalues))
    for i, X in enumerate(allXvalues):
        Y = allYvalues[i]
        totOD = allTotalODs[i]
        FitBounds = ((totOD-0.0001,ODsat-0.0001,N-0.0001),(totOD,ODsat,N)) # lower and upper bounds for totalOD, ODsat and N
        popt, pcov = scipy.optimize.curve_fit(fractionTransformed, X, Y, bounds = FitBounds)
        fit_ODsat = popt[1]
        fit_N = popt[2]
        Yfit[i] = fractionTransformed(X, totOD,fit_ODsat,fit_N)
    return Yfit


# fit all the concatenated data at once
FitBoundsAll = ((0.3,15),(0.6,50)) # lower and upper bounds for ODsat and N
poptAll, pcovAll = scipy.optimize.curve_fit(pTransWrapper, allXvalues, allYvalues, bounds = FitBoundsAll)
fit_ODsatAll = poptAll[0]
ODsatAll_error = np.round(np.sqrt(np.diag(pcov))[0],1)
fit_NAll = poptAll[1]
fit_NAll_error = np.round(np.sqrt(np.diag(pcov))[1],2)
print(fit_ODsatAll,fit_NAll)


#%% fit all the microscopy data + the plate reader data
# a useful function
def find_nearest(array, value):
    array = np.asarray(array)
    idx = (np.abs(array - value)).argmin()
...
allYvalues = allYvalues[1:]
allTotalODs = allTotalODs[1:]
lenMicroscopyData = len(allXvalues)

# now concatenate the X values of the plate reader experiment
platereaderXs = np.concatenate((MeanPerODPC5.index,MeanPerODPC4.index,MeanPerODPC6.index))
platereaderYs = np.concatenate((MeanPerODPC5['GFP'].values,MeanPerODPC4['GFP'].values,MeanPerODPC6['GFP'].values))
allXvalues = np.concatenate((allXvalues,platereaderXs))
allYvalues = np.concatenate((allYvalues,platereaderYs))


def pTransWrapper(allXvalues, ODsat, N,gpc5,gpc4,gpc6):
    Yfit = np.zeros(lenMicroscopyData)
    for i, X in enumerate(allXvalues[0:lenMicroscopyData]):
        #print(i)
        Y = allYvalues[i]
        totOD = allTotalODs[i]
        #FitBounds = ((totOD-0.0001,ODsat-0.0001,N-0.0001),(totOD,ODsat,N)) # lower and upper bounds for totalOD, ODsat and N
        ...
        # fit_ODsat = popt[1]
        # fit_N = popt[2]
        Yfit[i] = fractionTransformed(X, totOD,ODsat,N)
    
    #FitBounds = ((ODsat-0.0001,200),(ODsat,100000000)) # lower and upper bounds for totalOD, ODsat, N, and g    

    Xpc5 = platereaderXs[0:8]
    #Ypc5 = platereaderYs[0:8]
    # popt, pcov = scipy.optimize.curve_fit(tissueExpression2param, Xpc5, Ypc5, bounds = FitBounds)
    # fit_ODsat = popt[0]
    # fit_Ng = popt[1]
    # fit_N = popt[1]
    # fit_g = popt[2]
    fit = tissueExpression3param(Xpc5, ODsat,N,gpc5)  
    Yfit = np.concatenate((Yfit,fit))      
    
    Xpc4 = platereaderXs[8:16]
    #Ypc4 = platereaderYs[8:16]
    # popt, pcov = scipy.optimize.curve_fit(tissueExpression2param, Xpc4, Ypc4, bounds = FitBounds)
    # fit_ODsat = popt[0]
    # fit_Ng = popt[1]
    # fit_N = popt[1]
    # fit_g = popt[2]
    fit = tissueExpression3param(Xpc4, ODsat,N,gpc4)  
    Yfit = np.concatenate((Yfit,fit))
    
    Xpc6 = platereaderXs[16:]
    #Ypc6 = platereaderYs[16:]
    # popt, pcov = scipy.optimize.curve_fit(tissueExpression2param, Xpc6, Ypc6, bounds = FitBounds)
    # fit_ODsat = popt[0]
    # fit_Ng = popt[1]
    # fit_N = popt[1]
    # fit_g = popt[2]
    fit = tissueExpression3param(Xpc6, ODsat,N,gpc6)  
    Yfit = np.concatenate((Yfit,fit))
...
# fit all the concatenated data at once
FitBoundsAll = ((0.3,20,1000,1000,1000),(0.5,100,100000,100000,100000)) # lower and upper bounds for ODsat and N, gpc4, gpc5, gpc6
poptAll, pcovAll = scipy.optimize.curve_fit(pTransWrapper, allXvalues, allYvalues, bounds = FitBoundsAll)
fit_ODsatAll = poptAll[0]
ODsatAll_error = np.round(np.sqrt(np.diag(pcov))[0],1)
fit_NAll = poptAll[1]
...
fig, ax = plt.subplots()
fig.set_size_inches(2, 2)
Xpc5 = allXvalues[lenMicroscopyData:lenMicroscopyData+8]
Ypc5 = allYvalues[lenMicroscopyData:lenMicroscopyData+8]
Xpc4 = allXvalues[lenMicroscopyData+8:lenMicroscopyData+16]
Ypc4 = allYvalues[lenMicroscopyData+8:lenMicroscopyData+16]
Xpc6 = allXvalues[lenMicroscopyData+16:]
Ypc6 = allYvalues[lenMicroscopyData+16:]
fit5 = tissueExpression3param(Xpc5, fit_ODsatAll,fit_NAll,fit_g5)  
fit4 = tissueExpression3param(Xpc5, fit_ODsatAll,fit_NAll,fit_g4)  
fit6 = tissueExpression3param(Xpc5, fit_ODsatAll,fit_NAll,fit_g6)  
plt.plot(Xpc5,Ypc5,'ro')
plt.plot(Xpc5,fit5,'r-')
plt.plot(Xpc4,Ypc4,'bo')
plt.plot(Xpc5,fit4,'b-')
plt.plot(Xpc6,Ypc6,'go')
plt.plot(Xpc5,fit6,'g-')
plt.xscale('log')
plt.yscale('log')        
